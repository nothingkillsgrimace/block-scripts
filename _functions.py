...
#and selects 1 for each id, then returns the list.
#this is done because promos with A LOT of identical ids (CCF) can clog up promo selection
#this function tries to add more variety by only selecting 1 promo of each category
...
#this function will toss out filenames with a year that falls outside a specified year range
#this prevents commercials from different eras playing in certain blocks
def trim_years(list_fs,f_lengths,in_years):
    filtered_fs=[]
    filtered_ls=[]
    for f in list_fs:
        #if at least one of the in_years exists within the filename
        if np.max([yr in f for yr in in_years]):
            #add that filename to a list
            filtered_fs.append(f)
            #add the corresponding length of that filename to a separate list
            filtered_ls.append(int(f_lengths[list_fs==f]))
    
    #return the filtered filenames with their lengths (without having to recalculate)
...
#this function will calculate the length of each commercial break for a given show segments
def get_commercial_break_length(segments):
    #calculate the length of the video segments
    seg_lengths=length_vids(segments)
    #convert this to minutes
    total_lengths_minutes=seg_lengths.sum()/60
    #if the sum of all segments is less than 30 minutes
    if total_lengths_minutes<30:
        #commercials are padded to fill to the nearest 15 minute interval
        #shows that are less than 15 minutes pad up to 15, while shows >15 & <30 pad up to 30
        #EX: 8 minutes of show segments will pad up to 15; 21 minutes of show segments pads up to 30
        available_time=((math.ceil(total_lengths_minutes/15)*15)-total_lengths_minutes)*60
    else:
        #otherwise, shows greater than 30 pad up to the nearest 30 minute mark
        #EX: 70 minutes of show segments will add 20 minutes of commercials to get an even 90
    ...
    #as segments, but we will not be inserting commercial breaks between these segments
    #to account for this, we divide by the length of segments - 2 (intro + outro segment)
...
class Block_Assembler:
    ...
    def __init__(self,blocktype,paths,showtypes,ratings,seasonal):
        self.blocktype=blocktype
        self.allpaths=paths
        self.showtypes=showtypes
        self.tv_ratings=ratings
        self.season=seasonal
    ...
    def correct_clip_timeofyear(self,clip_dict,curmonth,months):
        ...
        for key in clip_dict.keys():
            ...
            #recursively go through and throw out any item in list that contains month
            for month in months:
    ...
    def correct_logo_timeofyear(self,logo_list,curmonth,months):
        curmonth_paths=np.array([clip for clip in logo_list if curmonth in clip])
        #recursively go through and throw out any item in list that contains month
        for month in months:
            logo_list=np.array([clip for clip in logo_list if month not in clip])
        logo_list=np.append(logo_list,curmonth_paths)
        return logo_list
        ...
        if past_episodes_used:
            if ((len(past_episodes_used)/len(show_files))>0.9) & (show_type=='episodic'):
                ...
                for past_episode in past_episodes_used:
                    ...
                    if past_episode[:-2] in show_files: #need to account for the \n at the end
    ...
    def add_logo_and_credits(self,clip_order,shows,block_num):
        ...
        # Extract first n-7 characters
        extracted = np.array([s[:-7] for s in clips_used])
        # Find unique entries
        unique_entries, unique_indices = np.unique(extracted, return_index=True)
        # Find last occurrence of each unique entry
        last_occurrences = np.array([np.where(extracted == entry)[0][-1] for entry in unique_entries])
        ...
        for cplace,cu in enumerate(clips_used):
            ...
            #ok so sometimes the last show clip includes the show AND credits, so this should be a catch-all check
            #to have the logo fade before credits or around when the credits start
            #logo usually doesn't stay on screen through credits
            if cplace in last_occurrences:
                offset = durr - 45
            else:
            ...
            #find out what the next show is for 'Coming Up Next' credit promos
            next_show='None'
            if cplace < len(clips_used)-1:
                for show in shows:
                    if show in clips_used[cplace+1]:
                        next_show = show
                        break

            #take care of instances where multiple logos are available
            if type(self.paths["logo"]) is list:
                seasonal_fixed_logos = self.correct_logo_timeofyear(self.paths["logo"], self.curmonth, self.months)
                sel_logo = random.choice(seasonal_fixed_logos)
            else:
                sel_logo = self.paths["logo"]
            
            #this takes care of credit stuff, video promos for nick & audio promos for cn
            if self.blocktype in ['Nickelodeon', 'Cartoon Network Powerhouse']:
                #isolate 'up next' credits, assembles remainder into list, select one at random
                normal_credits = np.array(self.clip_dict['credits'])[np.array(['Credits Next' not in clip for clip in self.clip_dict['credits']])]
                next_credits = np.array(self.clip_dict['credits'])[np.array(['Credits Next ' + next_show in clip for clip in self.clip_dict['credits']])]
                combined_credits = np.append(normal_credits,next_credits)
                sel_credit = random.choice(combined_credits)
                sel_credit_length = get_length(sel_credit)

                #credits will start so that there are 6 seconds left at the end of clip
                credits_start = int(durr - (sel_credit_length+10))
                #otherwise, need to set to 0 and it'll start right at beginning of clip
                credits_start = 0 if credits_start<0 else credits_start
                #sin curve of swipe requires even numbers, so minus 1 if start time is odd
                if credits_start%2!=0:
                    credits_start = int(credits_start-1)

            #shows that dont have normal credits should go here
            creditless_shows=['KaBlam', 'Looney Tunes']

            #probability of credits (1 - x)
            credit_probability = 0.3
            
            #if the relative place of the segment is right next to another, then don't add the tv rating
            if cu_rel_place[cplace]==1:
                if self.blocktype=='Disney' and cplace not in last_occurrences:
                    cmd = f'/unraid-scripts/logo/add_logo_no_tv_rating_disney.sh "{cu}" "{sel_logo}" "{str(durr)}" "{str(offset)}" "{clip_label}"'
                #if block is nickelodeon, the current show is the last in its order, and w/ % probability
                elif self.blocktype=='Nickelodeon' and cplace in last_occurrences and np.random.random() > credit_probability and cur_show not in creditless_shows:
                    cmd = f'/unraid-scripts/logo/add_logo_nick_credits_no_tv_rating.sh "{cu}" "{sel_logo}" "{sel_credit}" "{str(credits_start)}" "{str(credits_start+0.5)}" "{str(credits_start+30)}" "{str(credits_start+31)}" "{str(credits_start+1)}" "{str(credits_start+32)}" "{str(durr)}" "{str(offset)}" "{clip_label}"'
                elif self.blocktype=='Cartoon Network Powerhouse' and cplace in last_occurrences and np.random.random() > credit_probability and cur_show not in creditless_shows:
                    cmd = f'/unraid-scripts/logo/add_logo_cn_power_credits_no_tv_rating.sh "{cu}" "{sel_logo}" "{sel_credit}" "{str(credits_start)}" "{str(credits_start+0.5)}" "{str(credits_start+sel_credit_length)}" "{str(credits_start+sel_credit_length+1)}" "{str(durr)}" "{str(offset)}" "{clip_label}"'  
                else:
                    #command for the subprocess operation - NEED DOUBLE BRACKET QUOTES!
                    cmd = f'/unraid-scripts/logo/add_logo_no_tv_rating.sh "{cu}" "{sel_logo}" "{str(durr)}" "{str(offset)}" "{clip_label}"'
                process=subprocess.Popen(cmd, shell=True)
                process.wait()
                
            #otherwise we're likely coming in from commercial, so add the tv rating
            else:
                if self.blocktype=='Disney':
                    cmd = f'/unraid-scripts/logo/add_logo_disney.sh "{cu}" "{sel_logo}" "{tv_rating}" "{str(durr)}" "{str(offset)}" "{clip_label}"'
                #if block is nickelodeon, the current show is the last in its order, and w/ % probability
                elif self.blocktype=='Nickelodeon' and cplace in last_occurrences and np.random.random() > credit_probability and cur_show not in creditless_shows:
                    cmd = f'/unraid-scripts/logo/add_logo_nick_credits.sh "{cu}" "{sel_logo}" "{tv_rating}" "{sel_credit}" "{str(credits_start)}" "{str(credits_start+0.5)}" "{str(credits_start+30)}" "{str(credits_start+31)}" "{str(credits_start+1)}" "{str(credits_start+32)}" "{str(durr)}" "{str(offset)}" "{clip_label}"'
                elif self.blocktype=='Cartoon Network Powerhouse' and cplace in last_occurrences and np.random.random() > credit_probability and cur_show not in creditless_shows:
                    cmd = f'/unraid-scripts/logo/add_logo_cn_power_credits.sh "{cu}" "{sel_logo}" "{tv_rating}" "{sel_credit}" "{str(credits_start)}" "{str(credits_start+0.5)}" "{str(credits_start+sel_credit_length)}" "{str(credits_start+sel_credit_length+1)}" "{str(durr)}" "{str(offset)}" "{clip_label}"'
                else:
                    #command for the subprocess operation - NEED DOUBLE BRACKET QUOTES!
                    cmd = f'/unraid-scripts/logo/add_logo.sh "{cu}" "{sel_logo}" "{tv_rating}" "{str(durr)}" "{str(offset)}" "{clip_label}"'
                process=subprocess.Popen(cmd, shell=True)
                process.wait()
    ...
    def Boomerang(self,shows,type_dict,clip_dict,flag_dict,length_dict,
        ...
        boomerang_years=['2001','2002','2003']
        
        #get commercials & promos from years which CN Powerhouse aired
        clip_dict['promos_cn'],length_dict['promos_cn']=trim_years(clip_dict['promos_cn'],length_dict['promos_cn'],boomerang_years)
        clip_dict['commercials'],length_dict['commercials']=trim_years(clip_dict['commercials'],length_dict['commercials'],boomerang_years)
    ...
    def FOX(self,shows,type_dict,clip_dict,flag_dict,length_dict):
        ...
        for showpos,show in enumerate(shows):
            ...
            if type_dict[show]=='serial':
                ...
                #if show=='Simpsons':
                #    show_files=show_files[np.array(['Treehouse of horror' in segment for segment in show_files])==True]
    ...
    def Looney_Tunes(self, master_order, shows, showpos, clip_dict, flag_dict, length_dict, length_other):
        all_bumps = get_full_path(self.paths['looney_tunes_show'])
        fadeouts = all_bumps[np.array(['FadeOut' in b for b in all_bumps])==True]
        fadeins = all_bumps[np.array(['FadeIn' in b for b in all_bumps])==True]

        show_files=get_full_path(self.paths['looney_tunes_color'])
        show_files.sort()
        show_files=self.remove_past_episodes('Looney Tunes',show_files,'episodic',replace=True)

        show_segments = []
        length_of_show_clips = 0
        while length_of_show_clips < 45*60: #45 minutes
            random_sel = random.choice(show_files)
            length_of_show_clips += get_length(random_sel)
            show_segments.append(random_sel)
            show_files = show_files[np.array([f != random_sel for f in show_files])==True]
        ...
        for segpos,segment in enumerate(show_segments):    
            ...
            if segpos==0:
                master_order.append(all_bumps[np.array(['Intro Looney Tunes Show.mp4' in b for b in all_bumps])==True][0])
                length_other+=get_length(all_bumps[np.array(['Intro Looney Tunes Show.mp4' in b for b in all_bumps])==True][0])
            else:
                in_bump=random.choice(fadeins)
                master_order.append(in_bump)
                length_other+=get_length(in_bump)
                
            master_order.append(segment)
            
            if segpos!=len(show_segments)-1:
                out_bump=random.choice(fadeouts)
                master_order.append(out_bump)
                length_other+=get_length(out_bump)
            else:
                master_order.append(all_bumps[np.array(['Outro looney tunes show.mp4' in b for b in all_bumps])==True][0])
                length_other+=get_length(all_bumps[np.array(['Outro looney tunes show.mp4' in b for b in all_bumps])==True][0])
                master_order.append('BREAK END OF SHOW')
    ...
    def CN_Powerhouse(self,shows,type_dict,clip_dict,flag_dict,length_dict):
        ...
        for showpos,show in enumerate(shows):
            if show=='Top 5':
                ...
                length_of_other+=temp_length_other
                continue
            elif show=='Looney Tunes':
                master_order,temp_length_other=self.Looney_Tunes(master_order,shows,showpos,clip_dict,flag_dict,length_dict,length_of_other)
                length_of_other+=temp_length_other
                continue
    ...
    def Nickelodeon(self,shows,type_dict,clip_dict,flag_dict,length_dict):
        master_order=[]
        
        powerhouse_years=['1998','1999','2000','2001','2002','2003','2004','2005']
        
        #get commercials & promos from years which CN Powerhouse aired
        clip_dict['promos'],length_dict['promos']=trim_years(clip_dict['promos'],length_dict['promos'],powerhouse_years)
        clip_dict['commercials'],length_dict['commercials']=trim_years(clip_dict['commercials'],length_dict['commercials'],powerhouse_years)
        
        #trim out CN promos so there's 1 of each type
        clip_dict['promos'],length_dict['promos']=trim_promos(clip_dict['promos'],length_dict['promos'])
        ...
        #else:
        #    length_of_other=0
        length_of_other=0 #get rid of this if fixing/uncommenting above
        ...
        for showpos,show in enumerate(shows):
            ...
            generic_in=clip_dict['bumps'][np.array(['FadeIn Neutral' in bump for bump in clip_dict['bumps']])==True]
            generic_out=clip_dict['bumps'][np.array(['FadeOut Neutral' in bump for bump in clip_dict['bumps']])==True]
            ...
            #get an intro station id that matches the show about to be on
            matching_stations=clip_dict['ids'][np.array(['Intro '+show in bump for bump in clip_dict['ids']])==True]
            if len(matching_stations)==0:
                matching_stations=clip_dict['ids'][np.array(['Intro Neutral' in bump for bump in clip_dict['ids']])==True]
    ...
    def Disney(self,shows,type_dict,clip_dict,flag_dict,length_dict,seasonal=None):
        master_order=[]

        if seasonal:
            seasonal_files=get_full_path('/data/media/block_media/Broadcast_Shows/Disney/Seasonal/'+seasonal+'/')
            seasonal_files.sort()
            print(seasonal_files)

        for showpos,show in enumerate(shows):

            if seasonal:
                show_files=seasonal_files[np.array([show in ps for ps in seasonal_files])==True]
                print(show_files)
                ep_selector=random.choice(show_files)[:-8]
                print(show,ep_selector)
                show_segments=show_files[np.array([ep_selector in segment for segment in show_files])==True]
            ...
            #select all files in bumps folder that contain the word 'Ident'
            channel_idents=clip_dict['bumps'][np.array(['Ident' in f for f in clip_dict['bumps']])==True]
            #get fades to commercial bumps
            fadeout_bumps=clip_dict['bumps'][np.array(['FadeOut_'+show in f for f in clip_dict['bumps']])==True]
            #gets fades from commercial bumps
            fadein_bumps=clip_dict['bumps'][np.array(['FadeIn_'+show in f for f in clip_dict['bumps']])==True]

            for segpos,segment in enumerate(show_segments):

                potential_schedules_temp=None

                if segpos==0:
                    random_ident=random.choice(channel_idents)
                    while random_ident in master_order:
                        random_ident=random.choice(channel_idents)
                        
                    if showpos<=len(shows)-2:
                        if showpos not in np.arange(len(shows)-2,len(shows)):
                            potential_schedules_temp=clip_dict['bumps'][np.array(['Next_'+shows[showpos]+'_'+shows[showpos+1] in entry for entry in clip_dict['bumps']])==True]

                        if potential_schedules_temp:
                            random_schedule=random.choice(potential_schedules_temp)
                        else:
                            pass

                    if potential_schedules_temp:
                        length_of_other=length_vids(list((random_schedule,random_ident))).sum()
                    else:
                        length_of_other=get_length(random_ident)
                    ...
                    master_order.append(random_ident)
                    
                    if showpos<=len(shows)-2 and (potential_schedules_temp):
                        master_order.append(random_schedule)
                ...
                if segpos<len(show_segments)-1:
                    if len(fadeout_bumps):
                        sel_fadeout=random.choice(fadeout_bumps)
                    else:
                        sel_fadeout=None
                    if len(fadein_bumps):
                        sel_fadein=random.choice(fadein_bumps)
                    else:
                        sel_fadein=None

                    if sel_fadeout and sel_fadein:
                        length_of_other=length_vids(list((sel_fadeout,sel_fadein))).sum()
                    elif sel_fadeout and not sel_fadein:
                        length_of_other=get_length(sel_fadeout)
                    elif not sel_fadeout and sel_fadein:
                        length_of_other=get_length(sel_fadein)
                    elif not sel_fadeout and not sel_fadein:
                        length_of_other=0
    ...
    def generate(self,
        ...
        if user_defined_month:
            self.curmonth=user_defined_month        
        ...
        for key in self.clip_dict.keys():
            ...
            and key!='intro' and key!='outro' and key!='vg' and key!='station_ids' and key!='groovies' and key!='ids':
        ...
        elif self.blocktype=='Disney':
            master_order=self.Disney(showlist,self.showtypes,self.clip_dict,self.probability_dict,self.length_dict,self.season)
        elif self.blocktype=='Nickelodeon':
            master_order=self.Nickelodeon(showlist,self.showtypes,self.clip_dict,self.probability_dict,self.length_dict)
